...
const types_pkg_json_1 = require("types-pkg-json");
const upath_1 = require("upath");
const PropertyDictionary_1 = require("./Collections/PropertyDictionary");
const DumpLogic_1 = require("./DumpLogic");
...
class Package extends JSONObjectBase_1.JSONObjectBase {
    ...
    get DumpLogics() {
        return new Map([
            ...[
                ...
                "OptionalDependencies",
                "BundledDependencies"
            ].map((key) => [key, DumpLogic_1.DumpLogic.JSONObject]),
            ...[
                "Maintainers",
                "Contributors"
            ].map((key) => [key, DumpLogic_1.DumpLogic.JSONObjectArray])
        ]);
    }
    /**
    ...
    Normalize() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.FileName) {
                let packageRoot = upath_1.dirname(upath_1.resolve(this.FileName));
                if (yield fs_extra_1.pathExists(packageRoot)) {
                    ...
                    if (gitRoot) {
                        ...
                        if (upath_1.resolve(gitRoot) !== upath_1.resolve(packageRoot)) {
                            directory = upath_1.relative(gitRoot, packageRoot);
                        }
                    }
                    if (readmeFile) {
                        metadata.readme = (yield fs_extra_1.readFile(upath_1.join(packageRoot, readmeFile))).toString();
                    }
                }
            }
            normalize(metadata);
    ...
    ToJSON() {
        ...
        for (let entry of this.PropertyMap.entries()) {
            ...
            let generationLogic = GenerationLogic_1.GenerationLogic.Default;
            let dumpLogic = DumpLogic_1.DumpLogic.Default;
            if (this.DumpLogics.has(entry[1])) {
                dumpLogic = this.DumpLogics.get(entry[1]);
            }
            switch (dumpLogic) {
                case DumpLogic_1.DumpLogic.JSONObject:
                    value = value.ToJSON();
                    break;
                case DumpLogic_1.DumpLogic.JSONObjectArray:
                    value = value.map((obj) => obj.ToJSON());
                    break;
                default:
                    break;
            }
 ...
 */
Package.FileName = types_pkg_json_1.fileName;
