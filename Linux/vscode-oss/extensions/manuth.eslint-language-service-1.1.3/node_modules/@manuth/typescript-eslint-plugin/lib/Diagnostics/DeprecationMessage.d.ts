import type { CLIEngine } from "eslint";
import type { DiagnosticCategory, SourceFile } from "typescript/lib/tsserverlibrary";
import { Plugin } from "../Plugin";
import { Diagnostic } from "./Diagnostic";
/**
 * Represents a message about a deprecated rule.
 */
export declare class DeprecationMessage extends Diagnostic {
    /**
     * The deprecated rule that has been used.
     */
    private deprecatedRuleUse;
    /**
     * Initializes a new instance of the {@link DeprecationMessage `DeprecationMessage`} class.
     *
     * @param plugin
     * The plugin of the diagnostic.
     *
     * @param file
     * The file of the diagnostic.
     *
     * @param deprecatedRuleUse
     * The deprecated rule that has been used.
     *
     * @param category
     * The category of the diagnostic.
     */
    constructor(plugin: Plugin, file: SourceFile, deprecatedRuleUse: CLIEngine.DeprecatedRuleUse, category?: DiagnosticCategory);
    /**
     * Gets the deprecated rule that has been used.
     */
    get DeprecatedRuleUse(): CLIEngine.DeprecatedRuleUse;
    /**
     * @inheritdoc
     */
    get Message(): string;
}
