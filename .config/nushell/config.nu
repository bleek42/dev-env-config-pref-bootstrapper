...
# Neofetch
# display system info & specs when starting NuShell
def fetch_sys_info [] {
  neofetch
}

# // TO-DO make work
def writejson [filename, input] {
  $input | to json | save $filename
}

# To add entries to PATH (on Windows you might use Path), you can use the following pattern:
# let-env PATH = ($env.PATH | split row (char esep) | prepend '/some/path')
# let-env Path = ($env.Path | split row (char esep) | append  'C:\MongoDB\Tools\100\bin')

let-env Path = ($env.Path | split row (char esep) | append  'C:\MongoDB\Tools\100\bin')

# Aliases
# list by last modified, type (dir, file, symlink) at top
alias lt = (ls | sort-by modified -r | sort-by type)

# list only name
alias lname = (ls | get name)

# MySQL Shell
alias mysql = mysqlsh.exe
...
fetch_sys_info
source ~\AppData\Roaming\starship\init.nu